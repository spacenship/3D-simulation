...
ELEVATION_Y_CM = 8.0              # 레일-바닥 이격 거리: 8cm (붕 띄울 높이)
...
def get_pos(s):
    if s < 0:
        ...
        y = R_cycloid * (1 + math.cos(theta)) + ELEVATION_Y  # 레일만 8cm 공중 부양
        z = 0.0
        return vector(x, y, z)
    elif s <= L_loop:
        x = R_loop * math.sin(s / R_loop)
        y = R_loop * (1 - math.cos(s / R_loop)) + ELEVATION_Y # 레일만 8cm 공중 부양
        z = 0.04 * (s / L_loop) 
        return vector(x, y, z)
    else:
        s_exit = s - L_loop
        x = s_exit
        y = 0 + ELEVATION_Y                                  # 레일만 8cm 공중 부양
        z = 0.04 
        return vector(x, y, z)
...
# =====================================================================
scene = canvas(title='<b>3D Steel Coaster (True 8cm Elevated Structural Model)</b>',
               width=900, height=550,
               center=vector(0.05, R_loop + ELEVATION_Y/2, 0), background=vector(0.1, 0.1, 0.12))
scene.forward = vector(0.4, -0.2, -1.0) # 이격이 더 잘 보이도록 카메라 각도 미세 조정
scene.ambient = color.gray(0.4) 

# [수정 완벽 반영] 바닥판을 고도 오프셋 없이 0 지점에 깔아 8cm 이격을 시각화
# 두께 1cm짜리 상판의 상단 표면이 정확히 y = 0이 되도록 배치
ground = box(pos=vector(0.05, -0.005, 0.02), size=vector(1.6, 0.01, 0.5), color=vector(0.18, 0.18, 0.2))
...
# [구조물 추가] 레일이 떠받쳐져 있음을 보여주는 하부 지지 타워 프레임들 (y=0 지면에서부터 연결)
# 1) 최고점 스타팅 플랫폼 지지 타워 (y=0 에서 플랫폼 바닥까지)
cylinder(pos=vector(start_pos.x, 0, 0), axis=vector(0, start_pos.y, 0), radius=0.004, color=color.gray(0.5))

# 2) 원형 루프 진입부 지지 포스트 (x=0 구간 하부 받침대)
loop_bottom_pos = get_pos(0)
cylinder(pos=vector(0, 0, 0), axis=vector(0, loop_bottom_pos.y - R_ball, 0), radius=0.003, color=color.gray(0.4))

# 3) 최종 탈출 직진 코스 엔드 포스트 (엔드 레일 하부 받침대)
end_pos = get_pos(S_MAX)
cylinder(pos=vector(end_pos.x, 0, end_pos.z), axis=vector(0, end_pos.y - R_ball, 0), radius=0.003, color=color.gray(0.4))
